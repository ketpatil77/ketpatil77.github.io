...
export default function GeometricBlurMesh({
    ...
    const [currentShape, setCurrentShape] = useState(() => {
        const normalized = Math.trunc(initialShape);
        const safeIndex = Number.isFinite(normalized) ? normalized : 0;
        return ((safeIndex % shapes.length) + shapes.length) % shapes.length;
    });
    ...
    // Track pointer only inside the component to avoid global listeners.
    useEffect(() => {
        const container = containerRef.current;
        if (!container) return;
    ...
    useEffect(() => {
        const normalized = Math.trunc(initialShape);
        const safeIndex = Number.isFinite(normalized) ? normalized : 0;
        const nextShape = ((safeIndex % shapes.length) + shapes.length) % shapes.length;
    ...
    return (
        <div
            ref={containerRef}
            className={`relative w-full h-screen bg-black cursor-pointer overflow-hidden ${className ?? ''}`}
        >
            <canvas
                ref={canvasRef}
